...
def run_seed(seed: int) -> dict:
    ...
    metrics.update({"method": "M0", "seed": seed, "route": "基准"})
    return metrics


def main() -> None:
    rows = [run_seed(s) for s in SEEDS]
    save_metrics_csv(rows, OUT / "M0_各seed指标.csv")
    summary = (
        pd.DataFrame(rows)
        .groupby("method")[["pr_auc", "brier", "precision", "recall", "f1"]]
        .agg(["mean", "std"])
    )
    summary.to_csv(OUT / "M0_汇总_mean_std.csv", encoding="utf-8-sig")
    print(summary)
